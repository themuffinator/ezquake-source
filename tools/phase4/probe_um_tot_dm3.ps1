...
$out = 'E:\_RE\runtime\phase4-matrix\probe_um_tot_dm3'
...
$sp = Start-Process -FilePath $exe -ArgumentList "-condebug $slog -dedicated -allowmultiple -basedir $base -game ktx -port 28661 +maxclients 8 +sv_progtype 2 +sv_csqc_progname csprogs.dat +sv_antilag 1 +um tot +map dm3" -PassThru
Start-Sleep -Seconds 4
$cp = Start-Process -FilePath $exe -ArgumentList "-condebug $clog -allowmultiple -basedir $cbase -port 28691 -window -startwindowed +vid_fullscreen 0 -nosound +name phase4_human +developer 1 +cl_csqc 1 +exec phase4_onenter_addbot.cfg +connect 127.0.0.1:28661" -PassThru
...
Select-String -Path $clog -Pattern 'on_enter|botcmd|bot|entered the game|support|Command not known|Unknown command|Bots not enabled|UserMode|tot' | ForEach-Object { $_.Line }
Write-Output '---SERVER LINES---'
Select-String -Path $slog -Pattern 'bot|entered the game|removed|dropped|frag|killed|bro|UserMode|tot|k_fb_enabled' | ForEach-Object { $_.Line }
