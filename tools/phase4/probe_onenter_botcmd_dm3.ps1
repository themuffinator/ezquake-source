...
$out = 'E:\_RE\runtime\phase4-matrix\probe_onenter_botcmd_dm3'
...
$sp = Start-Process -FilePath $exe -ArgumentList "-condebug $slog -dedicated -allowmultiple -basedir $base -game ktx -port 28659 +maxclients 8 +sv_progtype 2 +sv_csqc_progname csprogs.dat +sv_antilag 1 +map dm3" -PassThru
Start-Sleep -Seconds 4
$cp = Start-Process -FilePath $exe -ArgumentList "-condebug $clog -allowmultiple -basedir $cbase -port 28689 -window -startwindowed +vid_fullscreen 0 -nosound +name phase4_human +developer 1 +cl_csqc 1 +exec phase4_onenter_botcmd.cfg +connect 127.0.0.1:28659" -PassThru
...
Select-String -Path $slog -Pattern 'bot|entered the game|Frog|frogbot|removed|dropped|support|disable|enable|frag|killed' | ForEach-Object { $_.Line }
